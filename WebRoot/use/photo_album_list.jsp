...
<table width="750" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    ...
    <%if(userid2==userid1){ %><img src="images/icon_infor.png" width="16" height="16" align="absmiddle" style="margin:0px 5px 0px 10px;"/><a href="userdo?param=singleuser&userid=<%=userid %>">个人资料</a><%} %>
    </strong></td>
  </tr>
</table>
...
<table width="789" border="0" align="center" cellpadding="0" cellspacing="0" >
  <tr>
    ...
    <form method="post" action="photoalbumlistdo?param=preadd&userid=<%=ub.getUserid() %>">
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td align="left" valign="top"><table width="85%" border="0" align="right" cellpadding="0" cellspacing="0">
          <tr>
            <td height="30"><strong class="f12_000">相册列表</strong>
            <label>
                  <%
					if (userid2==userid1) {
				%><input type="submit" value="创建新相册" style="margin-left:300px;" />
                  <%
                  	}
                  %>
            </label>
          ...
          <%
          	Collection photoalbum = (Collection) request
          			.getAttribute("photoalbum");
          	Iterator it = photoalbum.iterator();
          	while (it.hasNext()) {
          		PhotoalbumBean pab = (PhotoalbumBean) it.next();
          %>
          <tr>
            <td height="40" class="f12_t" style="border-bottom:1px #000 dotted;">
            <a href="/blog/use/photodo?albumid=<%=pab.getAlbumid()%>&userid=<%=pab.getUserid()%>">
            <%=pab.getAlbumname()%>
            </a>
			...
			if (userid2==userid1) {
			%>
			<a href = "photoalbumlistdo?param=del&albumid=<%=pab.getAlbumid() %>&userid=<%=pab.getUserid() %>">删除</a>&nbsp;&nbsp;&nbsp;
			<a href = "photoalbumlistdo?param=preupdate&albumid=<%=pab.getAlbumid()%>&userid=<%=pab.getUserid()%>">修改</a>
			<%
			}
			%>
            </td>
          ...
          <tr>
            ...
            <%if (page1>1){ %><a href="photoalbumlistdo?userid=<%=userid%>&page=<%=page1 - 1%>"><%} %>上一页<%if (page1>1){ %></a><%} %>&nbsp;&nbsp;&nbsp;&nbsp;<%if (page1<((count-1)/size+1)){ %><a href="photoalbumlistdo?userid=<%=userid%>&page=<%=page1 + 1%>"><%} %>下一页<%if (page1<((count-1)/size+1)){ %></a><%} %>
            <img src="images/arrow_2.png" width="13" height="12" align="absmiddle" />&nbsp;&nbsp;
